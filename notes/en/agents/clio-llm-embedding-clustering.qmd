...
In addition to saving embeddings, `prepare.py` computes 10-NN distance statistics, a TwoNN intrinsic-dimension estimate, and the LOF outlier ratio. The agent selects a base algorithm from that profile and the user's clustering goal. k-means is the default for regular, moderately sized dense data; HDBSCAN is available when noise and uneven density matter. For k-means, the script tests 0.7, 1.0, and 1.3 times the heuristic $\sqrt{8N}$ and selects among them with cosine silhouette.
...
The agent does not keep every cluster in conversational context. `probe.json` records the data profile and algorithm decision, `base.json` records initial clusters, and `level_1.json`, `level_2.json`, and later files preserve probes, neighborhoods, candidates, parents, assignments, and final clusters. Scripts own deterministic transformations; the agent writes back only the semantic decision for the current step.
...
## 4. The 20 Newsgroups Reconstruction
...
## 6. Why ACC Rises While NMI and ARI Fall
...
## 7. Why 0.657 Cannot Be Compared with the Paper's 94%
...
2. BrenchCC. [cilo-llm-embedding-clustering](https://github.com/BrenchCC/cilo-llm-embedding-clustering), commit `683cdf4`. 2026-08-12.
3. BAAI. [BAAI/bge-m3](https://huggingface.co/BAAI/bge-m3).
4. 周星星. [写了个 skill，用 LLM 结合 embedding 做聚类](https://www.xiaohongshu.com/discovery/item/6a792b2000000000330309db). Xiaohongshu. Included as project context and further reading; no image or unverified OCR content from the post is used here.
