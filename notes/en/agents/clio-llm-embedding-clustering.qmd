...
## 4. How to Invoke It: Start with Natural Language
...
The invocation can be as short as “cluster these queries by topic.” That states the task type, but it does not say whether the desired taxonomy should describe content, surface risk behavior, or produce labels suitable for storage. It also does not identify the data. The request can trigger the skill without giving the agent permission to skip questions that change the result.

To reduce turns, put the file, text column, clustering goal, and embedding model in the first message. The environment is not a business objective, but it determines where computation starts. If the current project has no stored choice, the agent confirms it separately before running anything.
