...
The problem I wanted to solve was concrete: given several thousand or tens of thousands of titles, queries, or short summaries, how can I produce a browsable topic tree instead of a column of opaque cluster IDs?

Running k-means is the easy part. The difficult questions come afterward. What should $K$ be? How should clusters be named? How should fine-grained topics be merged level by level? When should the process stop, and how can a person inspect the result? [*Clio: Privacy-Preserving Insights into Real-World AI Use*](https://arxiv.org/abs/2412.13678) offers an appealing division of labor: embeddings handle scale, while an LLM extracts semantics, labels clusters, and builds a hierarchy. I turned that clustering idea into the [llm-embedding-clustering](https://github.com/BrenchCC/cilo-llm-embedding-clustering) Agent Skill and evaluated it on a labeled 20 Newsgroups sample.
...
## 1. Clio Addresses a Conflict Between Analysis and Privacy

Model providers can access real usage data, but access does not make unconstrained analysis acceptable. Conversations can contain identity, health, employment, and business information. Human review also exposes reviewers to sensitive material repeatedly, and the daily volume of messages makes manual synthesis impractical even before the ethical concerns are considered.
...
I therefore preserved the clustering idea without describing the project as a privacy-preserving platform. Omitting one unsupported adjective from the README is more responsible than presenting an unimplemented safety property as a default capability.

## 3. Turning the Paper into an Agent Skill
...
Two version anchors matter. The reconstruction experiment ran at commit `683cdf4`; when I updated this note, remote `main` had advanced to `890b66a`. Both revisions use the same clustering algorithms and semantic-decision flow, so the previous experiment still represents the current method. The new revision tightens the runtime contract: before computation begins, the agent must obtain an explicit conda, uv, or venv choice, store the project environment configuration, and read it back for validation.

| Evidence role | Version and date | How this note uses it |
| --- | --- | --- |
| 20 Newsgroups experiment | `683cdf4`, 2026-08-12 | Metrics, timings, the `196 → 60 → 20` hierarchy, and report captures |
| Current usage guide | `890b66a`, 2026-08-13 | Installation, environment gate, work-directory contract, and outputs |
| Version check | Clustering method unchanged; runtime gate updated | Does not misreport an environment-protocol update as an algorithmic gain |
...
### 3.4 Why Not Give Every Source Text Directly to the LLM?

An apparently simpler implementation would place all source texts in context and ask the LLM to generate a taxonomy from scratch. That can work for a few dozen demo items. At larger scales it encounters context limits, cost, order sensitivity, and coverage problems simultaneously. The model may create detailed categories for early samples and use a few broad parents for later ones. There is also no explicit assignment table for investigating a missed long-tail topic.

Base clustering acts as compression. The 4,806 documents become 196 local units, and the agent sees samples, names, and descriptions from each unit. Higher levels operate on 196 cluster representations rather than rereading every post. The cost is that a bad base assignment cannot be split later; the hierarchy can only recombine erroneous units. This constraint helps explain why semantic merging can repair class-level alignment without repairing NMI and ARI globally.

## 4. Using the Skill from a Natural-Language Request
...
After installation, the user does not need to remember internal pipeline arguments. The interface is the coding agent, not an individual script. In practice, I put four facts in the request: the file, text column, clustering goal, and embedding model.
...
That sentence supplies every business input. If the current project has no environment configuration, the latest skill still asks whether to use conda, uv, or venv; conda requires an environment name. Only after the choice is stored does the agent validate dependencies and inspect data.

![Invocation flow from a natural-language request to final deliverables](../../assets/clio-llm-embedding-clustering/skill-invocation-en.svg)

### 4.1 How Much to Put in the Prompt

The [companion Zhihu article](https://www.zhihu.com/question/2027303590776063616/answer/2041949740292650311) does not begin with a feature inventory. It begins with a failure that readers can reproduce: asking one LLM context to cluster tens of thousands of texts eventually runs into the context limit and produces forgetting, duplicate names, and degraded decisions late in the prompt. Only then does it state the division of labor: embeddings absorb scale; the LLM handles semantics. This guide follows that practical order by showing a request first and explaining the mandatory follow-up questions afterward.

The invocation can be as short as “cluster these queries by topic.” A short request triggers the skill, but it does not remove mandatory questions. Supplying every result-changing choice in the first message reduces turns.

| Request | What the agent already knows | What it still needs |
| --- | --- | --- |
| “Cluster these queries by topic.” | The task is topic clustering | Environment, path, text column, goal, and embedding model |
| “Cluster `data.xlsx:text` into a hierarchy for content-topic distribution with bge-m3.” | Path, column, goal, and local model | Environment only when the current project lacks configuration |
| “Use an online embedding API to discover risk topics.” | The goal is risk discovery and embeddings come from an API | Environment, path, column, endpoint, key source, model name, and a minimal client example |

The clustering goal must be explicit. On the same support queries, content distribution may produce refund, logistics, and product-quality topics. Risk discovery instead emphasizes abuse, repeated-refund exploitation, or threats. Recommendation labeling favors names suitable for a downstream label store. This is not stylistic wording; it changes the taxonomy objective.

### 4.2 The Environment Gate Is an Execution Contract

Commit `890b66a` promotes the environment question to a hard gate. The agent may not infer the answer from the active shell, and it may not begin computation before asking. It stores the choice as project environment configuration and reuses it until the user explicitly requests a change.

| Choice | Runtime location | Best fit |
| --- | --- | --- |
| conda | A named existing environment | The project already pins a Conda CUDA/PyTorch stack |
| uv | An isolated environment in the current project | Fast setup managed by uv |
| venv | An isolated environment in the current project | Standard-library isolation without adding uv |

The Zhihu article documents an earlier usage snapshot and describes uv as the first-run path. Current commit `890b66a` instead offers conda, uv, or venv. This note follows the current execution contract rather than copying outdated installation details from the article.

That extra question prevents two practical failures: installing dependencies into the wrong environment and writing runtime state into the installed skill. The latter can make separate projects share `state/`, allowing one experiment to overwrite another experiment's JSON and embeddings.

### 4.3 Separate the Skill Directory from the Run

The current specification distinguishes the installed skill from the current project. The former supplies a reusable execution protocol, deterministic compute stages, semantic-decision templates, and dependency declarations. The latter owns the environment configuration, intermediate state, and final outputs. Results from propose, dedup, assign, and rename must return to one canonical per-level state chain; parallel work must not create a second side-channel record.

![Responsibility boundary between the installed skill and the current project](../../assets/clio-llm-embedding-clustering/runtime-layout-en.svg)

After a run, I do not judge success from whether the UMAP looks attractive. The three artifact groups answer different review questions:

| Artifact | Inspect first | Primary use |
| --- | --- | --- |
| `out/result.xlsx` | Source-to-leaf mapping, parent-child hierarchy, and cluster sizes | Statistics, sampling, and downstream business integration |
| `out/report.html` | Algorithm rationale, stopping decisions, final tree, cases, and UMAP | Human review of process and semantic boundaries |
| Intermediate state directory | Candidates, assignments, and pre/post-rename records | Locate the failing level and rerun locally on cached embeddings |

That is the difference from a clustering API that returns only labels. The skill automates execution without hiding the evidence needed to audit what the agent did.

## 5. The 20 Newsgroups Reconstruction
...
The Zhihu article also presents a 4,806-document 20 Newsgroups run, but reports a `137 → 48 → 20` hierarchy and different evaluation numbers. It does not provide the same sample hash and per-level state as the local report, so I treat it as an independent experimental snapshot rather than combining it with this note's `196 → 60 → 20`, 0.631, and 0.657. Every experimental claim below remains grounded in the local artifacts.

One small preprocessing detail is worth preserving. Eleven documents contained 21 control characters prohibited by Excel. The bge-m3 token sequences were identical before and after cleanup, so the operation made the workbook writable without changing embedding inputs. That statement is more useful for reproducibility than a generic claim that the data was “cleaned.”
...
The environment used Python 3.10.0, PyTorch 2.13.0, sentence-transformers 5.7.0, and scikit-learn 1.7.2. The bge-m3 revision was `5617a9f61b028005a4858fdac845db406aefb181`, stored in a local 2.14 GiB model directory. Embedding took 39 minutes 11 seconds, hierarchy building took 11 minutes 27 seconds, and the complete run took 50 minutes 38 seconds. Roughly 77% of wall time was embedding, which makes vector caching materially useful when iterating on hierarchy prompts.

## 6. How `196 → 60 → 20` Emerged
...
## 7. Why ACC Rises While NMI and ARI Fall

The final comparison is the most informative part of this reconstruction.
...
For me, reliability is not established by whether one generated taxonomy looks plausible. The intermediate state must remain inspectable, repeated runs must be measured for stability, and negative metrics must be published beside positive ones. Only with that evidence can an agent's semantic judgment become part of a maintainable engineering process.

That trade may be worthwhile when an analyst needs a navigable topic map. It is not enough when the objective is maximum partition consistency. Privacy is an even clearer boundary: the current implementation cannot borrow Clio's conclusion. Before processing user conversations, the aggregation thresholds and auditing stages that appear peripheral to clustering must return to the center of the system.
...
4. 周星星. [AI 的 SKILL 究竟是什么，应该如何应用？](https://www.zhihu.com/question/2027303590776063616/answer/2041949740292650311). Zhihu. Used as project context, a usage-narrative reference, and an independent experiment snapshot; its images and evaluation numbers are not reused here.
