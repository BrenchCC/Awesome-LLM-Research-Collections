...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), page 1; cropped from the vector PDF at 5× scale, approximately 360 DPI.*

## Summary and Core Assessment

The most memorable Kimi K3 figures are 2.8T total parameters, 104.2B active parameters, and a one-million-token context window. The report’s coherent technical story, however, is not simply that all three numbers grew. It redesigns information flow in four places. KDA supplies recurrent memory over long sequences while periodic Gated MLA layers preserve global retrieval. Block Attention Residuals let each layer select from earlier blocks. Stable LatentMoE compresses expert computation into a 3,584-dimensional latent space and expands the routed pool to 896 experts. MoonViT-V2 places image and text tokens under a shared next-token objective from the start of pre-training. All four changes address one engineering problem: preserve tractable training and serving while the model handles longer, deeper, sparser, and more interactive trajectories.

My assessment has four parts. First, K3’s architectural value lies in coordination, not in one isolated module. The reported “2.5× scaling efficiency” belongs to the combined K3 architecture, data, and training recipe relative to K2. The paper does not contain the ablation needed to allocate that factor among KDA, AttnRes, and LatentMoE. Second, 1M is a trained and runnable maximum window, not proof that every task uses one million tokens effectively. The primary BrowseComp result uses 300K compaction, while a raw full-window run without context management scores slightly lower. Third, post-training does more than add a generic RL stage: it trains nine policies across three domains and three reasoning-effort levels, then distills them into one served model. Fourth, open weights make architectural study and deployment possible, but the training corpus, complete data pipeline, all environments, and cluster software were not released with the checkpoint. “Open weights” should not be rewritten as “fully open and reproducible.”
...
## 2. From K2 to K3: Capacity, Active Compute, and Multiaxial Information Flow
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 2, page 3; 5× crop.*
...
*Custom diagram based on §2 and Figure 2 of the [K3 report](https://arxiv.org/abs/2607.24653).*
...
## 3. The Sequence Axis: A 3:1 Mixture of KDA and Gated MLA

KDA continues the delta-rule recurrence. For one attention head, $S_t$ is a matrix state updated at each token, $\alpha_t$ is a channel-wise retention factor, and $\beta_t$ controls write strength. The recurrence is:
...
The first term decays the old state by channel and uses $I-\beta_tk_tk_t^\top$ to erase content aligned with the current key. The second writes the new key–value association. Unlike full attention, KDA does not preserve head-specific K/V entries for every historical token; history is compressed into a fixed-shape state. During training it is recurrent across chunks and parallel within each chunk, containing cache growth on long sequences.

K3 does not replace every layer with linear recurrence. Each block contains three KDA layers and one Gated MLA layer, and a final MLA layer guarantees global attention at the top of the backbone. The periodic full-attention layer has a clear role. The recurrent state provides inexpensive accumulation, while MLA retains content-addressable access to per-token latents, compensating for fixed-state losses in exact recall and rare-evidence retrieval. A 3:1 mixture does not mean that linear attention has superseded softmax. It places their cache curves and retrieval properties inside the same backbone.

The chunkwise form also explains why KDA is not a slow token-by-token loop during training. An incoming state carries information from previous chunks. Within a chunk, cumulative decay, query–key interactions, and pseudo-values are expressed as matrix operations; the output sums a cross-chunk state read and causal intra-chunk interaction. Recurrence remains at chunk boundaries while most work within the chunk stays parallel. The difficult part is reciprocal cumulative decay: a run of near-zero retention factors can push scaling outside finite precision. Lower-bounded decay targets that numerical failure.
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 3, page 5; 5× crop.*
...
*Custom diagram based on §2.1 of the [K3 report](https://arxiv.org/abs/2607.24653) and the [vLLM K3 engineering report](https://vllm-project.github.io/2026/07/27/k3.html).*

## 4. The Depth Axis: How Block AttnRes Rewrites the Residual Stream
...
*Source: [Attention Residuals](https://arxiv.org/abs/2603.15031), Figure 1, page 1; 5× crop.*

Block AttnRes compresses depth history into block summaries. Ordinary residual accumulation remains inside a block, while only one representation survives for cross-block access. K3 uses eight blocks of twelve layers and counts the embedding as a ninth retrievable state. This loses layer-level retrieval granularity but reduces the representations that must stay live and cross pipeline stages from $L$ to $N$. It is useful to think of the mechanism as a block index over a depth cache rather than another sequence-attention layer: it changes where a layer obtains its input without changing token-level causal order.
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 4, page 7; 5× crop.*
...
*Source: [LatentMoE](https://arxiv.org/abs/2601.18089), Figure 1, page 2; 5× crop from the NVIDIA paper PDF.*
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 6, page 9; 5× crop.*
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 7, page 11; 5× crop.*
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Table 1, page 11; 5× crop.*
...
## 7. One-Million-Token Context: NoPE, Curriculum, and Effective Use
...
*Custom diagram based on §3.4 of the [K3 report](https://arxiv.org/abs/2607.24653).*
...
## 8. Post-Training: Distilling Nine Expert Policies into One Model
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 8, page 13; 5× crop.*
...
*Custom diagram based on §4.1 of the [K3 report](https://arxiv.org/abs/2607.24653).*

MOPD evaluates student-generated on-policy tokens with a dense signal from the appropriate teacher. For domain $d$, effort $e$, and current token $y_t$, the reward is a clipped log-ratio between teacher and student probabilities:
...
## 9. Agentic Environments and Long-Trajectory Infrastructure
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 9, page 15; 5× crop.*
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 12, page 23; 5× crop.*
...
*Source: vLLM, [“Serving Kimi K3 at Frontier Scale”](https://vllm-project.github.io/2026/07/27/k3.html), original web image, accessed 2026-08-01.*
...
## 10. Evidence Boundaries for Evaluation, Cost, and Engineering Cases
...
*Source: [Kimi K3 technical report](https://arxiv.org/abs/2607.24653), Figure 13, page 32; 5× crop. Cost uses API prices listed by the paper at evaluation time.*
...
![A dated Artificial Analysis snapshot of Kimi K3 showing intelligence, speed, price, and context-window fields.](../../assets/kimi-k3-open-frontier-intelligence/artificial-analysis-k3-snapshot.png){fig-alt="Screenshot of the Artificial Analysis Kimi K3 max page on August 1 2026 showing Intelligence Index 57, about 35.9 output tokens per second, 3 dollar input and 15 dollar output price, and a one-million-token context"}

*Source: [Artificial Analysis Kimi K3 page](https://artificialanalysis.ai/models/kimi-k3/), 2× browser screenshot, accessed 2026-08-01. Rankings, speed, pricing, and sample counts are dynamic.*
