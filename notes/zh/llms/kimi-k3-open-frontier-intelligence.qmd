...
KDA 使用 delta rule，也就是“边读取、边擦除、边写入”的更新规则。对一个注意力头，$S_t$ 是持续更新的矩阵记忆，$\alpha_t$ 决定旧信息保留多少，$\beta_t$ 决定新信息写入多强。公式如下：
...
这条公式可以读成三步：先衰减旧记忆，再擦除与当前 key 重合的部分，最后写入新的 key–value 关系。key 可以理解为检索标签，value 是要保存的内容。完整注意力会保存每个历史 token 的键和值；KDA 把它们压进固定大小的 $S_t$。因此，上下文继续变长时，KDA 的状态大小不会按 token 数量线性增长。

K3 没有把全部注意力层都换成 KDA。每个块使用 3 层 KDA 和 1 层 Gated MLA，最后一层还会再做一次全局 MLA。KDA 适合用固定状态长期积累信息；MLA 虽然需要按 token 增加缓存，但仍能直接按内容搜索历史位置。两者放在一起，是为了兼顾缓存成本和精确检索。这个 3:1 结构并不表示线性注意力已经完全取代 Softmax 注意力。

训练时，KDA 不会逐 token 串行计算。它先把序列切成多个块（chunk）：块与块之间传递记忆状态，块内部则改写成矩阵运算，因此大部分计算仍可并行。难点在累计衰减。如果连续很多步的保留系数都接近零，后续计算所需的倒数会变得非常大，超出低精度浮点数的安全范围。下界衰减正是用来限制这个数值范围。
...
Block AttnRes 用“分块摘要”降低成本。块内仍做普通残差相加，跨块只保存一个汇总表示。K3 每 12 层组成一个块，共 8 个块，并把输入嵌入（embedding）也当作一个可检索状态。这样做会失去逐层选择的细度，但长期保存和跨流水线阶段传输的状态从 $L$ 个层输出降到 $N$ 个块输出。它更像深度方向的分块索引：它改变一层从哪里取信息，不改变 token 之间的因果顺序。
...
LatentMoE 先把 token 的 7168 维特征压缩到 3584 维，再让专家在这个低维空间中计算，最后投影回 7168 维。这里的潜在空间（latent space）就是压缩后的内部特征空间。专家并行时，GPU 之间需要做全互联通信（all-to-all），也就是互相交换被路由到不同专家的 token。维度减半后，专家权重读取和通信量大约按 $d/\ell$ 缩小，节省的预算可以换成更多专家。K3 使用 896 个路由专家，每个 token 选择 16 个，另有 2 个始终参与计算的全宽共享专家。
...
K3 所说的“原生视觉”，是指文本、图片和视频从预训练开始就共同做下一 token 预测。它不是先训练纯语言模型，再在后面接一个视觉编码器。MoonViT-V2 有 401M 参数、27 层，把图像切成 $14\times14$ 的小块，并用 12 个注意力头处理，再通过一个轻量投影层接入 7168 维的共享表示。报告显示，从零训练的 MoonViT-V2 相比用 SigLIP 初始化的 MoonViT-3D，梯度更低、尖峰更少，视觉评测接近。这个结果只适用于论文设置，不能推成“预训练视觉编码器普遍没有用”。
