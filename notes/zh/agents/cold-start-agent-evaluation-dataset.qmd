...
title: "没有线上数据，如何为 Agent 构建冷启动评测集"
description: "从能力地图、人工金种子、受控变异、状态 Oracle、可重置环境和发布门禁出发，构建上线前可复核的 Agent 冷启动评测集。"
order: 1
...
本文聚焦冷启动评测集的构建。关于 `pass@k`、`pass^k`、LLM-as-a-Judge、置信区间和双层回归的进一步讨论，可参见[《如何科学评测 AI Agent：指标、环境、裁判与回归闭环》](scientific-agent-evaluation.qmd)。

![从业务能力地图和评测矩阵收敛到少量人工金种子](../../assets/cold-start-agent-evaluation-dataset/capability-to-seeds-zh.svg){width="94%" fig-align="center" fig-alt="冷启动入口流程图：业务材料形成能力地图，再按用户目标、复杂度、输入质量、系统状态和风险等级构造评测矩阵，最后选择二十到五十条人工金种子"}

_图 1：冷启动评测不是从「请生成 200 道题」开始，而是从业务能力与风险覆盖开始。_
...
![金种子经过受控变异、程序验收和人工复核形成扩充评测集](../../assets/cold-start-agent-evaluation-dataset/controlled-mutation-zh.svg){width="95%" fig-align="center" fig-alt="受控扩写流程图：金种子进入语言、参数边界、信息缺失、权限、工具故障、多轮和对抗变异，再经过语义去重、规则校验、可解性检查、分布检查与人工复核"}

_图 2：LLM 可以扩写表述和状态组合，但不能自行改变业务规则、预期结果或安全边界。_
...
![Outcome、Behavior、Response 三层 Oracle 依次连接确定性检查、规则检查、LLM Judge 和安全门禁](../../assets/cold-start-agent-evaluation-dataset/three-oracles-zh.svg){width="94%" fig-align="center" fig-alt="三层评测图：Outcome Oracle 检查最终状态，Behavior Oracle 检查工具、参数和业务不变量，Response Oracle 检查解释与忠实性；安全违规直接进入失败发布门禁"}

_图 3：能由状态和规则确定的部分优先自动判断，LLM Judge 只处理开放表达。_
...
![金种子、覆盖集、挑战集、回归集、Canary 与隐藏集经过发布和生产反馈形成数据生命周期](../../assets/cold-start-agent-evaluation-dataset/dataset-lifecycle-zh.svg){width="95%" fig-align="center" fig-alt="数据生命周期图：六个数据桶分工进入开发与发布，随后经过内部试用、影子流量、小比例灰度和生产轨迹，失败挖掘与分布漂移再更新回归集"}

_图 4：数据桶按职责拆分；公开开发数据与封存发布数据不能混用。_
