...
真正调用时可以从一句很短的话开始，例如「帮我把这批 query 做一下主题聚类」。这句话足以表达任务类型，却没有说明要看内容主题、风险行为还是可落库标签，也没有告诉 Agent 数据在哪里。Skill 可以被触发，但不能因为追求“一句话完成”就跳过会改变结果的澄清。

更省往返的做法，是在第一条消息里把文件、文本列、聚类目的和 embedding 模型写全。环境不属于业务目标，却决定计算从哪里启动；当前项目还没有记录时，Agent 会在运行前单独确认。
