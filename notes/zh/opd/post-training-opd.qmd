...
![Thinking Machines Lab 的 Qwen3-8B-Base 实验：full finetuning 与不同 LoRA rank 从 SFT checkpoint 继续进行 on-policy distillation 时的 AIME'24 分数和附加计算量。](../../assets/post-training-opd/opd-qwen3-8b.png){#fig-tml-opd-qwen3-8b fig-align="center" width="88%"}

*来源：[Thinking Machines Lab, On-Policy Distillation](https://thinkingmachines.ai/blog/on-policy-distillation/) 中的无编号网页图；网页无页码。横轴是作者按 FLOPs 折算的附加计算量；9–30 倍是该文相对 SFT 外推得到的条件性估算，不是 OPD 的通用加速比。*
...
![Qwen3 Technical Report Table 21：Qwen3-8B 上 off-policy 蒸馏、直接强化学习与 on-policy distillation 的性能、pass@64 和 GPU hours 对比。](../../assets/rl-matters/qwen3-rl-vs-opd-table-21.png){#fig-qwen3-table21-post-training-opd fig-align="center" width="88%"}

*来源：[Qwen3 Technical Report，Table 21，第 21 页](https://arxiv.org/abs/2505.09388)。括号内为 pass@64；表中的 1,800 GPU hours 不包含训练强教师所需的前置成本。*
...
![Thinking Machines Lab 的 self-distillation 对比：左图是 RL 与不同 LoRA rank OPD 的 AIME'24 训练曲线，右图是学生相对教师的 reverse KL。](../../assets/post-training-opd/opd-self-distilltation.png){#fig-tml-self-distillation fig-align="center" width="90%"}

*来源：[Thinking Machines Lab, On-Policy Distillation](https://thinkingmachines.ai/blog/on-policy-distillation/) 中的无编号网页图；网页无页码。这组 50–100 倍估算来自同架构 self-distillation 设置，与 Qwen3 Table 21 不是同一次实验。*
